...
RC qeCustomTest02() {
    ...
    // Test: INLJoin with varchar

    std::cerr << std::endl << "***** In QE Test Case Custom 02 *****" << std::endl;

    RC rc = success;

    // Prepare the iterator and condition
    rm.createIndex("leftvarchar", "B");
    rm.createIndex("rightvarchar", "B");
    auto *leftIn = new IndexScan(rm, "leftvarchar", "B");
    auto *rightIn = new IndexScan(rm, "rightvarchar", "B");
    ...
    auto *bnlJoin = new INLJoin(leftIn, rightIn, cond);
    ...
    *(float *) value2.data = 35.0; // C[10.0-35.0], then B: a ... zzzzzzzzzzzzzzzzzzzzzzzzzz
    ...
    free(value2.data);
    free(data);
    rm.destroyIndex("leftvarchar", "B");
    rm.destroyIndex("rightvarchar", "B");
    return rc;
}
...
int main() {
    ...
    if (qeCustomTest02() != success) {
        std::cerr << "***** [FAIL] QE Test Case Custom 02 failed. *****" << std::endl;
        return fail;
    } else {
        std::cerr << "***** QE Test Case Custom 02 finished. *****" << std::endl;
        return success;
    }
}
